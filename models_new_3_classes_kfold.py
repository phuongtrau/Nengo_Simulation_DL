...
exp_i_data = export_data_exp_i_3_classes("experiment-i",preprocess=True)
...
for train,test in kfold.split(x_train,y_train):
    ...
    dense = tf.keras.layers.Dense(units=3)(flatten)
    ...
    if do_training:
        with nengo_dl.Simulator(net_train, minibatch_size=64,seed=0) as sim:
            ...
            sim.fit(
                {converter.inputs[inp]: x_train[train]},
                {converter.outputs[dense]: y_train[train]},
                validation_data=(
                    {converter.inputs[inp]: x_train[test]},
                    {converter.outputs[dense]: y_train[test]},
                ),
                epochs=10,
                #callbacks=[model_checkpoint_callback],
            )
            # save the parameters to file
            path_params = "SNN_KFOLD/SNN_BED_KFOLD_{}".format(fold_no)
